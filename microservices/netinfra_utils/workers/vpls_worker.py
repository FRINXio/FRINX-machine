...
def device_create_vpls_remote(task):
    vpls_config = create_vpls_remote_request(task)
...
def create_vpls_remote_request(task):
    instance, vpls_body = create_vpls_base_request(task, vpls_response_template)

    for i, ifc in enumerate(task['inputData']['interface']):
    ...
    for r in task['inputData']['remote_ip']:
        ...
        cp2['endpoints']['endpoint'][0]['remote']['config']['remote-system'] = r.remote_ip
...
def start(cc):
    ...
    cc.register('VPLS_device_create_remote')
    cc.start('VPLS_device_create_remote', device_create_vpls_remote, False)
