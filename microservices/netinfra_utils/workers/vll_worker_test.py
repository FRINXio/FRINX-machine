...
def test_parse_service_remote():
    service_id, vccid, devices = vll_worker.parse_service_vccid({"inputData": {
...
def test_aggregate_remote_l2p2p():
    ...
    aggregated = vll_worker.aggregate_l2p2p_remote(partial_services)
    print(aggregated)
    assert len(aggregated) is 2

    aggregated = vll_worker.aggregate_l2p2p_remote(partial_services, lambda x: x['vccid'])
    print(aggregated)
    assert len(aggregated) is 2

    aggregated = vll_worker.aggregate_l2p2p_remote(partial_services, lambda x: x)
    print(aggregated)
